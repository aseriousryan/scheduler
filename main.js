...
let secFunc;
let contactNum;
let cronJobs = {}; // Variable to hold an array of cron job instances
...
async function sendResponse(message, contactNum, imgBuffer) {
	try {
		...
		// form.append("img", imgBuffer || "", { filename: "image.jpg" }); // Add filename for the image
...
app.post("/trigger-api", (req, res) => {
	...
	const imgBuffer = req.file ? req.file.buffer : undefined; // img is optional, and it could be undefined

	if (!message && !imgBuffer) {
		return res.status(400).json({ message: "Must atleast insert message or image" });
	}
	...
	newCronSchedules.forEach((schedule) => {
		const newCronJob = cron.schedule(schedule, async () => {
			...
			try {
				...
				const response = await sendResponse(chatApiResponse, contactNum, imgBuffer);
